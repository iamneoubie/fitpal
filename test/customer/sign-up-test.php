 ...
 * Validates the customer multi-step registration flow
 *
 * @package FitPal
 * @version 2.8
 */
...
function renderLiveFormSection($baseUrl, $defaultCollapsed = true) {
    ...
    echo "<p style='font-size:13px;color:#718096;margin:5px 0;'>URL: " . htmlspecialchars($baseUrl . '/customer/pages/sign-up.php') . "</p>";
    echo "<div class='step-indicator'>";
    echo "<span class='step-badge active'>Step 1: Personal Info</span>";
    echo "<span class='step-badge'>Step 2: Dietary</span>";
    echo "<span class='step-badge'>Step 3: Allergies</span>";
    echo "<span class='step-badge'>Step 4: Fitness</span>";
    echo "</div>";
    echo "<iframe src='" . $baseUrl . "/customer/pages/sign-up.php'></iframe>";
    ...
    echo "<a href='" . $baseUrl . "/customer/backend/handlers/sign-up-handler.php' target='_blank' class='btn-test'>Test Handler</a>";
    echo "</div>";
    echo "</div>";
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .step-badge { padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; background: #edf2f7; color: #718096; }
        .step-badge.active { background: #4299e1; color: white; }
        ...
        @media (max-width: 640px) {
            ...
            .step-badge { font-size: 10px; padding: 3px 8px; }
        }
        ...
        @media (max-width: 400px) {
            ...
            .step-badge { font-size: 9px; padding: 2px 6px; }
        }
    </style>
</head>
<body>
...
<div class='test-header'>
    ...
    <p>Validates the customer multi-step registration flow</p>
...
if (file_exists($dbFile)) {
    ...
    if (strpos($content, '$database_connection') !== false) {
        ...
        try {
            ...
            $stmt = $test_connection->query("SHOW TABLES LIKE 'customer_profile'");
            $hasProfileTable = $stmt->rowCount() > 0;
            testResult('Customer Profile Table', $hasProfileTable, $hasProfileTable ? 'Exists' : 'Missing', 'Database');
            
            $stmt = $test_connection->query("SHOW TABLES LIKE 'financial_account'");
            $hasFinancialTable = $stmt->rowCount() > 0;
            testResult('Financial Account Table', $hasFinancialTable, $hasFinancialTable ? 'Exists' : 'Missing', 'Database');
...
$handlerExists = file_exists($customerPath . '/backend/handlers/sign-up-handler.php');
...
if (file_exists($signUpPage)) {
    $content = file_get_contents($signUpPage);
    testResult('Page Readable', true, 'File loaded', 'Page Structure');
    
    preg_match('/<form[^>]*id=["\']registerForm["\'][^>]*>/i', $content, $formMatches);
    testResult('Form (registerForm)', !empty($formMatches), !empty($formMatches) ? 'Found' : 'Missing', 'Page Structure');
    ...
    preg_match_all('/<div[^>]*class=["\'][^"\']*register-step["\'][^>]*id=["\']step(\d+)["\'][^>]*>/i', $content, $stepMatches);
    $stepCount = count($stepMatches[1] ?? []);
    testResult('Multi-Step Form (4 steps)', $stepCount === 4, 'Found ' . $stepCount . ' steps', 'Page Structure');
    
    preg_match('/<div[^>]*class=["\'][^"\']*register-progress["\'][^>]*>/i', $content, $progressMatches);
    testResult('Progress Indicator', !empty($progressMatches), !empty($progressMatches) ? 'Found' : 'Missing', 'Page Structure');
    
    $expectedFields = ['first_name', 'last_name', 'birthdate', 'gender', 'email', 'contact_number', 'username', 'password', 'confirm_password', 'terms'];
    
    $foundFields = [];
    preg_match_all('/<input[^>]*name=["\']([^"\']+)["\'][^>]*>/i', $content, $inputMatches);
    $foundFields = array_merge($foundFields, $inputMatches[1] ?? []);
    preg_match_all('/<select[^>]*name=["\']([^"\']+)["\'][^>]*>/i', $content, $selectMatches);
    $foundFields = array_merge($foundFields, $selectMatches[1] ?? []);
    ...
    preg_match('/<div[^>]*id=["\']dietaryOptions["\'][^>]*>/i', $content, $dietMatches);
    testResult('Dietary Preferences Section', !empty($dietMatches), !empty($dietMatches) ? 'Found' : 'Missing', 'Page Structure');
    
    preg_match('/<div[^>]*id=["\']allergyOptions["\'][^>]*>/i', $content, $allergyMatches);
    testResult('Allergies Section', !empty($allergyMatches), !empty($allergyMatches) ? 'Found' : 'Missing', 'Page Structure');
    
    preg_match('/<select[^>]*id=["\']fitness_goal["\'][^>]*>/i', $content, $fitnessMatches);
    testResult('Fitness Goal Select', !empty($fitnessMatches), !empty($fitnessMatches) ? 'Found' : 'Missing', 'Page Structure');
    
    preg_match('/<div[^>]*id=["\']notifierModal["\'][^>]*>/i', $content, $modalMatches);
    testResult('Notification Modal', !empty($modalMatches), !empty($modalMatches) ? 'Found' : 'Missing', 'Page Structure');
    
    $hasSignUpJS = strpos($content, 'sign-up.js') !== false;
    testResult('sign-up.js Included', $hasSignUpJS, $hasSignUpJS ? 'Found' : 'Missing', 'Page Structure');
    
    preg_match('/<form[^>]*action=["\']([^"\']+)["\'][^>]*>/i', $content, $actionMatches);
    if (!empty($actionMatches[1])) {
        $formAction = $actionMatches[1];
        $isCorrect = strpos($formAction, 'sign-up-handler.php') !== false;
...
if (file_exists($handlerPath)) {
    ...
    $hasPasswordHash = strpos($handlerContent, 'password_hash') !== false;
    testResult('password_hash()', $hasPasswordHash, $hasPasswordHash ? 'Found' : 'Missing', 'Handler');
    
    $hasTransaction = strpos($handlerContent, 'beginTransaction') !== false;
    testResult('Transaction Support', $hasTransaction, $hasTransaction ? 'Found' : 'Missing', 'Handler');
    
    $hasJsonResponse = strpos($handlerContent, 'application/json') !== false;
    testResult('JSON Response', $hasJsonResponse, $hasJsonResponse ? 'Found' : 'Missing', 'Handler');
    
    $hasNameValidation = strpos($handlerContent, 'preg_match') !== false && strpos($handlerContent, 'first_name') !== false;
    testResult('Name Validation', $hasNameValidation, $hasNameValidation ? 'Found' : 'Missing', 'Handler');
    
    $hasEmailValidation = strpos($handlerContent, 'FILTER_VALIDATE_EMAIL') !== false;
    testResult('Email Validation', $hasEmailValidation, $hasEmailValidation ? 'Found' : 'Missing', 'Handler');
    
    $hasDuplicateCheck = strpos($handlerContent, 'Duplicate entry') !== false || strpos($handlerContent, 'checkEmail') !== false;
    testResult('Duplicate Check', $hasDuplicateCheck, $hasDuplicateCheck ? 'Found' : 'Missing', 'Handler');
    
    $hasProfileInsert = strpos($handlerContent, 'customer_profile') !== false;
    testResult('Customer Profile Insert', $hasProfileInsert, $hasProfileInsert ? 'Found' : 'Missing', 'Handler');
    
    $hasFinancialInsert = strpos($handlerContent, 'financial_account') !== false;
    testResult('Financial Account Insert', $hasFinancialInsert, $hasFinancialInsert ? 'Found' : 'Missing', 'Handler');
    
    $redirectCorrect = strpos($handlerContent, '../../pages/sign-in.php') !== false || strpos($handlerContent, 'sign-in.php') !== false;
    testResult('Redirect to sign-in.php', $redirectCorrect, $redirectCorrect ? 'Correct path' : 'Check redirect path', 'Handler');
    
    $hasRespondError = strpos($handlerContent, 'function respondError') !== false;
    testResult('respondError Function', $hasRespondError, $hasRespondError ? 'Found' : 'Missing', 'Handler');
} else {
...
if (file_exists($jsPath)) {
    ...
    testResult('sign-up.js Readable', true, 'File loaded', 'JavaScript');
    ...
    $hasFormRef = strpos($jsContent, 'registerForm') !== false;
    testResult('Form Reference', $hasFormRef, $hasFormRef ? 'Found' : 'Missing', 'JavaScript');
    
    $hasStepNav = strpos($jsContent, 'goToStep') !== false;
    testResult('Step Navigation', $hasStepNav, $hasStepNav ? 'Found' : 'Missing', 'JavaScript');
    
    $hasValidation = strpos($jsContent, 'validateStep') !== false;
    testResult('Validation Function', $hasValidation, $hasValidation ? 'Found' : 'Missing', 'JavaScript');
    ...
    $hasFetch = strpos($jsContent, 'fetch(') !== false;
    testResult('fetch() Usage', $hasFetch, $hasFetch ? 'Found' : 'Missing', 'JavaScript');
    
    $hasNotifier = strpos($jsContent, 'notifierModal') !== false;
    testResult('Notification Modal', $hasNotifier, $hasNotifier ? 'Found' : 'Missing', 'JavaScript');
    
    $hasOptionCards = strpos($jsContent, 'option-card') !== false || strpos($jsContent, 'optionCard') !== false;
    testResult('Option Cards', $hasOptionCards, $hasOptionCards ? 'Found' : 'Missing', 'JavaScript');
} else {
...
if (file_exists($cssPath)) {
    ...
    testResult('sign-up.css Readable', true, 'File loaded', 'CSS');
    
    $hasStepStyles = strpos($cssContent, 'register-step') !== false;
    testResult('Step Styles', $hasStepStyles, $hasStepStyles ? 'Found' : 'Missing', 'CSS');
    
    $hasProgressStyles = strpos($cssContent, 'register-progress') !== false;
    testResult('Progress Styles', $hasProgressStyles, $hasProgressStyles ? 'Found' : 'Missing', 'CSS');
    
    $hasOptionStyles = strpos($cssContent, 'option-card') !== false;
    testResult('Option Card Styles', $hasOptionStyles, $hasOptionStyles ? 'Found' : 'Missing', 'CSS');
