...
//  * Version 3.1 - Fixed modal and quantity control issues
//  * 
//  * @package FitPal
//  * @version 3.1
//  */
...
//     // DOM REFERENCES
//     // ============================================
//     var queuePanelWrapper = document.getElementById('queuePanelWrapper');
//     var queuePanel = document.getElementById('queuePanel');
//     var queuePanelInner = document.getElementById('queuePanelInner');
...
//         if (queuePanelWrapper) {
//             if (totalItems > 0) {
//                 queuePanelWrapper.classList.add('has-items');
//                 queuePanelWrapper.style.display = 'block';
//             } else {
//                 queuePanelWrapper.classList.remove('has-items');
//                 queuePanelWrapper.style.display = 'none';
//                 if (isOpen) {
//                     closePanel();
//                 }
//             }
...
//         // Visual feedback
//         if (queuePanelInner) {
//             queuePanelInner.style.borderColor = 'var(--accent)';
//             setTimeout(function() {
//                 queuePanelInner.style.borderColor = '';
//             }, 500);
//         }
//     };
...
//     // ============================================
//     // MODAL EVENT BINDING - FIXED
//     // ============================================
//     function bindModalEvents() {
//         // Remove Item Modal - Close button
//         if (removeModalClose) {
//             var newClose = removeModalClose.cloneNode(true);
//             removeModalClose.parentNode.replaceChild(newClose, removeModalClose);
...
//             var newCancel = removeModalCancel.cloneNode(true);
//             removeModalCancel.parentNode.replaceChild(newCancel, removeModalCancel);
...
//             var newConfirm = removeModalConfirm.cloneNode(true);
//             removeModalConfirm.parentNode.replaceChild(newConfirm, removeModalConfirm);
...
//         // Remove Item Modal - Overlay click
//         var removeOverlay = removeModal ? removeModal.querySelector('.queue-modal-overlay') : null;
//         if (removeOverlay) {
//             var newOverlay = removeOverlay.cloneNode(true);
//             removeOverlay.parentNode.replaceChild(newOverlay, removeOverlay);
...
//         // Cancel Order Modal - Close button
//         if (cancelModalClose) {
//             var newCancelClose = cancelModalClose.cloneNode(true);
//             cancelModalClose.parentNode.replaceChild(newCancelClose, cancelModalClose);
//             newCancelClose.addEventListener('click', function(e) {
//                 e.preventDefault();
//                 e.stopPropagation();
//                 closeCancelModal();
//             });
...
//             var newCancelCancel = cancelModalCancel.cloneNode(true);
//             cancelModalCancel.parentNode.replaceChild(newCancelCancel, cancelModalCancel);
//             newCancelCancel.addEventListener('click', function(e) {
//                 e.preventDefault();
//                 e.stopPropagation();
//                 closeCancelModal();
//             });
...
//         // Cancel Order Modal - Confirm button
//         if (cancelModalConfirm) {
//             var newCancelConfirm = cancelModalConfirm.cloneNode(true);
//             cancelModalConfirm.parentNode.replaceChild(newCancelConfirm, cancelModalConfirm);
//             newCancelConfirm.addEventListener('click', function(e) {
//                 e.preventDefault();
//                 e.stopPropagation();
//                 confirmCancelOrder();
//             });
...
//         // Cancel Order Modal - Overlay click
//         var cancelOverlay = cancelModal ? cancelModal.querySelector('.queue-modal-overlay') : null;
//         if (cancelOverlay) {
//             var newCancelOverlay = cancelOverlay.cloneNode(true);
//             cancelOverlay.parentNode.replaceChild(newCancelOverlay, cancelOverlay);
//             newCancelOverlay.addEventListener('click', function(e) {
//                 if (e.target === this) {
//                     closeCancelModal();
//                 }
//             });
...
//         console.log('Queue Panel v3.1 - Fixed modal and quantity controls');
//     }
...
/**
 * FitPal Customer Queue Panel JavaScript
 * Version 3.7 - Working transitions + reliable event binding
 * 
 * @package FitPal
 * @version 3.7
 */
...
(function() {
    ...
    function saveQueue(queue) {
        sessionStorage.setItem('fitpal_queue', JSON.stringify(queue));
        currentQueue = queue.slice(); // copy to avoid reference issues
        renderQueue();
        updatePanelVisibility();
        updateCheckoutButton();
    }
    ...
    function updatePanelVisibility() {
        ...
        // Ensure the wrapper is always block so transitions work
        queuePanelWrapper.style.display = 'block';

        // Remove both classes before applying the correct one
        queuePanelWrapper.classList.remove('has-items', 'empty');
        ...
        if (totalItems > 0) {
            ...
            queuePanelWrapper.classList.add('empty');
            if (isOpen) {
                closePanel();
            }
        }
...
var imageSrc = (item.image && item.image.trim() !== '') ? item.image : defaultIcon;
    ...
    // ============================================
    // BIND ITEM EVENTS (cloneNode method – reliable)
    // ============================================
    function bindItemEvents() {
        // Quantity minus buttons
        document.querySelectorAll('.qty-minus').forEach(function(btn) {
    ...
    window.addToQueue = function(productId, name, price, quantity, image, stock, restaurantName, branchName) {
        ...
        if (existingIndex >= 0) {
            ...
            saveQueue(currentQueue);
            syncWithServer();
            // Open the panel immediately after adding
            openPanel();
        }
    ...
    // ============================================
    // MODAL EVENT BINDING (cloneNode – reliable)
    // ============================================
    function bindModalEvents() {
        // Remove Item Modal - Close button
        if (removeModalClose) {
    ...
    function init() {
        ...
        // Ensure the wrapper is always block – override any inline style
        if (queuePanelWrapper) {
            queuePanelWrapper.style.display = 'block';
            // Start with empty class if no items
            var initialQueue = getQueue();
        ...
        // Listen for add-to-cart form submissions (only if they don't have onsubmit)
        document.addEventListener('submit', function(e) {
            var form = e.target.closest('.add-to-cart-form');
            if (!form) return;

            // If the form has an onsubmit attribute, let it handle the submission
        ...
        console.log('Queue Panel v3.7 initialized with transitions');
    }
